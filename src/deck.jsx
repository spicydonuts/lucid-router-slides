...
import transitionGroupAddon from 'react-addons-transition-group'
...
const TransitionGroup = Radium(transitionGroupAddon)
...
class Deck extends React.Component {
  ...
  _toggleOverviewMode() {
    ...
    this.context.router.navigate('/' + (this.context.slide) + suffix)
  }
  ...
  _goToSlide(e) {
    if (e.key === 'spectacle-slide') {
      ...
      if (this._checkFragments(this.context.slide, data.forward)) {
        this.context.router.navigate('/' + (data.slide) + this._getSuffix())
      }
    }
  }
  _prevSlide() {
    ...
    if (this._checkFragments(this.context.slide, false) || this.context.overview) {
      if (slide > 0) {
        this.context.router.navigate('/' + this._getHash(slide - 1) + this._getSuffix())
        localStorage.setItem('spectacle-slide',
